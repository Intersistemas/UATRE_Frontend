...
//         </div>  
//         {process.env.REACT_APP_PROTOCOL !== "https" && <h6 style={{textShadow: '1px 1px 15px yellow'}} className="float-end">{process.env.REACT_APP_SERVER.toUpperCase()}</h6>}
...
//#endregion 
  ...
  //CODIGO ORIGINAL, COMENTADO PORQUE NO SE UTILIZA
  // //#region Capturo errores de login
  // useEffect(() => {
  //   if (error) {
  //     setMensajeError(error.message);
...
useEffect(() => {
  if (error) {
    ...
    // Si el error es "No address found.", redirigir al registro con el usuario
    if (error.message === "No address found.") {
      navigate("/Registro", { 
        state: { 
          usuario: enteredCUIT,
          mensaje: "Usuario no encontrado. Complete el registro." 
        }
      });
      return;
    }
...
//#endregion
  ...
  const processLogIn = async (userObject) => {
    await authContext.login(
     ...
     console.log("Usuario logueado 1: ", userObject);
