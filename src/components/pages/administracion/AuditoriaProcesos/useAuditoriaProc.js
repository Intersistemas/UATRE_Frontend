...
import GestionOSTable from "./AuditoriaProcTable";
import GestionOSForm from "../../gestionOS/GestionOSForm";
...
import GestionOSComprobante from "../../gestionOS/GestionOSComprobante";
...
const onDownloadComprobanteGestion = (data = {}) => {
  ...
  pdf(doc)
    ...
    .then((blob) => {
      ...
      a.href = url;
      a.download = `ConstanciaGestion_${data.nroGestion}.pdf`;
      ...
      <GestionOSForm
        ...
        disabled={(() => {
          ...
          if (["M"].includes(list.selection.request)) {
            ...
            r.gestionEstado =
              list.selection.record.gestionEstadoId === 3 ? true : false;
        ...
        onValidate={(confirm) => {
          ...
          if (["A", "M"].includes(list.selection.request)) {
            ...
            if (!record.apellidoTitular)
              errors.apellidoTitular = "Dato requerido";
            ...
            if (
              ...
              errors.direccionesEmailDestino = "Email inválido";
            if (
              record.medioGestion == "telefono" &&
              (!record.telefono || record.telefono.length <= 6)
            )
              errors.telefono = "Dato requerido";
            if (
              record.medioGestion == "telefono" &&
              (!record.telefono || !matchIsValidTel(record.telefono))
            )
              errors.telefono = "Teléfono invalido";
            if (record.medioGestion == "telefono" && !record.resultadoLlamada)
              errors.resultadoLlamada = "Dato requerido";

            if (!record.gestionRubroId || record.gestionRubroId == 0)
              errors.gestionRubro = "Dato requerido";
            if (!record.gestionSubRubroId || record.gestionSubRubroId == 0)
              errors.gestionSubRubro = "Dato requerido";
            if (!record.gestionEstadoId || record.gestionEstadoId == 0)
              errors.gestionEstado = "Dato requerido";
            if (!record.gestionSituacionId || record.gestionSituacionId == 0)
              errors.gestionSituacion = "Dato requerido";
            if (
              !record.gestionAreaOspreraId ||
              record.gestionAreaOspreraId == 0
            )
              errors.gestionAreaOsprera = "Dato requerido";

            if (
              record.telefonoContacto?.length &&
              !matchIsValidTel(record.telefonoContacto)
            )
              errors.telefonoContacto = "Teléfono de contacto inválido";
            if (
              record.telefonoContacto2?.length &&
              !matchIsValidTel(record.telefonoContacto2)
            )
              errors.telefonoContacto2 = "Teléfono de contacto 2 inválido";
            if (
              record.emailContacto?.length &&
              !ValidarEmail(record.emailContacto)
            )
              errors.emailContacto = "Email de contacto inválido";
            if (
              record.emailContacto2?.length &&
              !ValidarEmail(record.emailContacto2)
            )
              errors.emailContacto2 = "Email de contacto 2 inválido";

            if (!record.atencionesPrevias)
              errors.atencionesPrevias = "Dato requerido";
            if (record.atencionesPrevias === "S" && !record.tipoPrestador) {
              errors.tipoPrestador = "Dato requerido";
            }
            if (
              record.atencionesPrevias === "S" &&
              !record.conCoberturaOsprera
            ) {
...
export default useAuditoriaProc;