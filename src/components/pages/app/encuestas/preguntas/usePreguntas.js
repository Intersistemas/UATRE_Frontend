...
import AutoridadesForm from "./PreguntasForm";
...
const usePreguntas = () => {
	...
	const pushQuery = useQueryQueue((action, params) => {
		console.log('usePreguntas_pushQuery_action',action);
		console.log('usePreguntas_pushQuery_params',params);
		switch (action) {
			case "GetList": {
				return {
					config: {
	...
	useEffect(() => {
		...
		console.log("usePreguntas effet_List",list)

		/*{setList((o) => {
			
			const selection = {
				...selectionDef,
				record: list ?? list?.at(0),
			};
			if (selection.record)
				selection.index = list.indexOf(selection.record);
			return {
				...o,
				loading: null,
				//pagination: { index, size, count },
				data: list.sort((a,b)=> a.ordenPregunta - b.ordenPregunta),
				error: null,
				selection,
			};
		})}
*/
	...
	const requestChanges = useCallback((type, payload = {}) => {
		console.log("usePreguntas_payload",payload)
		switch (type) {
			case "selected": {
				return setList((o) => ({
					...o,
