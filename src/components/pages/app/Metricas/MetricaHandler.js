...
const MetricaHandler = () => {
	...
	// Seccional de la localidad
	const [seccionalMetrica, setSeccionalMetrica] = useState({
		loading: null,
		params: {},
		data: null,
		error: null,
	...
	useEffect(() => {
		if (!seccionalMetrica.loading) return;
		const changes = {
			loading: null,
			data: null,
			error: null,
		...
		pushQuery({
			action: "GetSeccionalMetrica",
			params: { ...seccionalMetrica.params },
			onOk: async (rta) => {
				if (!Array.isArray(rta))
					return console.error("Se esperaba un arreglo.", { rta });
				changes.data = rta.splice(0, 1).find(() => true);
