...
const DenunciaDetails = (props) => {
    ...
    useEffect(() => {
        if (tab === "denuncia" && data.derivadoAId) {
            fetchDerivacionInfo(data.derivadoAId);
        } else {
            setDerivacionInfo(null);
        }
    }, [data.derivadoAId, tab, fetchDerivacionInfo]);
    ...
    const getDerivacionDescripcion = useCallback(() => {
        ...
        if (tipoDerivacion === "seccional") {
            ...
            return derivacionInfo.refDelegacionDescripcion || 'Delegación no disponible';
        } else {
            const seccional = derivacionInfo.seccionalDescripcion;
            const delegacion = derivacionInfo.refDelegacionDescripcion;
            
            if (seccional && delegacion) {
                return `${seccional} - ${delegacion}`;
            } else if (seccional) {
                return seccional;
            } else if (delegacion) {
                return delegacion;
    ...
    }, [derivacionInfo, data.derivadoAId, data.derivadoATipo, validar]);
                    ...
                    <Grid className={`${styles.fondo} ${styles.grupo}`} col>
                        ...
                        <Grid className={styles.grupo} col full>
                            <Grid className={styles.contenido} col>
                                ...
                                <Grid width>
                                    <InputMaterial label="Derivado A Tipo" value={validar(data.derivadoATipo)}/>
