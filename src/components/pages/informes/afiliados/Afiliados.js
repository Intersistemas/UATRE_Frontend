...
import useGeneracionExcel from "components/hooks/useGeneracionExcel"; // ← NUEVO
...
const Afiliados = ({ onClose = onCloseDef }) => {
  ...
  // NUEVO: GET /Seccional para construir mapa delegacionId -> provinciaId
  ...
  // NUEVO: todas las seccionales (para armar ámbito por provincia)
  const [seccionalesAll, setSeccionalesAll] = useState([]);

  // NUEVO: mapa delegacionId -> provinciaId
  ...
  const valueForExport = (col, row) => {
    ...
    // 1) Si definís un excelFormat en la columna, úsalo (override específico para Excel)
    if (typeof col.excelFormat === "function") return col.excelFormat(v, row);

    // 2) Por defecto, usamos lo MISMO que ves en la UI
    if (typeof col.formatter === "function") return col.formatter(v, row);

    // 3) Si no hay formatter, recién ahí usamos csvFormat
  ...
  // === NUEVO: Cargar /Seccional (GET) => map delegación->provincia y todas las seccionales ===
  useEffect(() => {
    ...
    setSeccionalesAllQuery((o) => ({
      ...
      onLoad: ({ ok, error }) => {
        ...
        setMapDelegProv(m);
        setSeccionalesAll(ok); // <- GUARDAMOS el array completo de seccionales
  ...
  // === NUEVO: cuando cambia la provincia, filtrar delegaciones y resetear seccional ===
  useEffect(() => {
    ...
    setDelegacionSelect((prev) => {
      ...
      const filtradas =
        provIdSel == null
          ...
          : base.filter((d) => {
            const delegId = d?.id;
            const provFromMap = mapDelegProv.get(delegId);
            return Number(provFromMap) === provIdSel;
          });
  ...
  useEffect(() => {
    ...
    const onLoad = ({ query, ok, error }) => {
      ...
      if (pageIndex < pages) {
        ...
        changes.data = ambito ? data.filter((r) => ambito.includes(r.id)) : data;
        changes.optionsSrc = seccionalSelectOptions(changes, ambitoUsuario);
        changes.selectedDef =
          changes.optionsSrc.length === 1 ? changes.optionsSrc[0] : seccionalSelectTodos;
    ...
    setMotivosBajaQuery((o) => ({
      ...
      onLoad: ({ ok, error }) => {
        ...
        setMotivosBajaSelect((o) => {
          ...
          n.selectedDef =
            n.optionsSrc.length === 1 ? n.optionsSrc[0] : motivosBajaSelectTodos;
          n.selected = n.selectedDef;
          return n;
        });
      },
  ...
  useEffect(() => {
    ...
    setEstadosQuery((o) => ({
      ...
      onLoad: ({ ok, error }) => {
        let data = [];
        if (Array.isArray(ok))
          data = ok.filter((estadoSolicitud) => estadoSolicitud?.tipo === "Afiliados");
        setEstadoSelect((o) => {
          const n = {
            ...o,
            loading: null,
  ...
  useEffect(() => {
    ...
    setSeccionalSelect((o) => {
      const n = {
        ...o,
        reload: true,
        refDelegacionId: selected === delegacionSelectTodos ? 0 : selected?.value,
  ...
  //#region Cambia select provincia (REEMPLAZO COMPLETO)
  useEffect(() => {
    if (provinciaSelect.loading) return;
    ...
    // 1) Actualizar filtros: quitar deleg/seccional (evita OR), setear provincia numérica, y sacar ambitoTodos
    setFiltros((f) => {
      const n = { ...f };
      delete n.ambitoDelegaciones;
      delete n.ambitoSeccionales;
      ...
      if (provIdSel == null) {
        ...
        // (opcional) podrías restaurar ambitoTodos acá desde init.filtros si tu flujo lo requiere
      } else {
        n.ambitoProvincias = { ids: [provIdSel] }; // numérico
        delete n.ambitoTodos;                      // clave para que no anule el filtro
      }
      return n;
    });

    // 2) Refiltrar opciones de delegación usando el mapa delegación->provincia
    if (Array.isArray(delegacionSelect.data) && delegacionSelect.data.length && mapDelegProv.size) {
      ...
      // 3) Resetear seccional (queda vacío hasta que elijas delegación)
      setSeccionalSelect((prev) => ({
        ...prev,
        selected: seccionalSelectTodos,
        optionsSrc: [],
  ...
  // ===== EXCEL: misma lógica que CSV, pero generando .xlsx =====
  useEffect(() => {
  ...
  const onAplicaFiltros = () => {
    ...
    // 1) Si hay seccional puntual: usar solo esa y limpiar provincia/delegación para evitar OR
    if (seccionalIdElegida != null) {
      ...
      if (provIdNum != null && delegId != null) {
        // 2) Provincia + Delegación => intersección
        ...
        delete filtrosDepurados.ambitoDelegaciones; // evitamos OR
        delete filtrosDepurados.ambitoProvincias;   // ya está implicado por las seccionales
        ...
        // "Todas" en provincia y sin delegación/seccional -> no tocar ámbitos
    ...
    // 5) Normalizar arrays de IDs a number[]
    ...
    // 6) Si hay filtros específicos, remover ambitoTodos
  ...
  return (
    <Modal size="xl" centered show>
      <Modal.Header className={modalCss.modalCabecera}>Afiliados</Modal.Header>
      <Modal.Body>
          ...
          <Grid width gap="inherit">
            <Grid grow>
              <SearchSelectMaterial
                ...
                helperText={motivosBajaSelect.loading ?? motivosBajaSelect.error}
            ...
            <Grid width="200px">
              <Button
                ...
                disabled={
                  ...
                  // si hay provincia elegida y aún no está listo el catálogo
                  (provinciaSelect?.selected !== provinciaSelectTodos && !seccionalesAllReady)
                }
            ...
            <Grid width="200px">
              <Button
                className="botonAzul"
                disabled={JSON.stringify(filtros) === JSON.stringify(init.filtros) }
