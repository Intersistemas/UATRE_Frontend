...
import { Document, Page as RenderPage, Text, View, Image } from "@react-pdf/renderer";
...
import RenderGrid from "components/ui/Grid/Grid.js";
...
const FormaPagoPDF = ({
	...
	const Grid = ({ style, ...p }) => (
		<RenderGrid row render={View} style={{ ...style, ...styles.border }} {...p} />
	);
	...
	const Header = ({ pagina = "" }) => (
		<Grid col>
			...
			<Grid col width grow style={{...styles.borderLeft, ...styles.borderTop, ...styles.borderRight}}>
	...
	const Footer = ({ data = {} }) => {
		...
		if (modelo === 1) {
			...
			identificador.valor = formaPago.identificadorDeudaInformado;
			barcode = null;
		}
		if (barcode)
			barcode = <Image src={barcode} style={{ width: 400, height: 45 }} />;
		identificador.valor = Formato.Mascara(identificador.valor, "#".repeat(10));
		return (
			<Grid col width grow style={{...styles.borderLeft, ...styles.borderRight, ...styles.borderBottom}}>
				<Grid><Text>&nbsp;</Text></Grid>
				<Grid><Text>&nbsp;</Text></Grid>
				<Grid width>
					<Grid
					...
					>
						<Text style={styles.titulo}>{identificador.nombre}:</Text>
						<Text>{identificador.valor}</Text>
					</Grid>
					<Grid grow />
					<Grid
						width="345"
	...
	const Page = ({ children, header, footer, ...x }) => (
		<RenderPage style={styles.page} size="A5" orientation="landscape" {...x}>
			<Grid col grow>
				<Header {...header} />
				<Grid col gap="10px">
					{children}
	...
	pages.push(
		...AsArray(formasSplit).map(({ index, pages, data }) => {
			return (
				<Page header={{ pagina: `Página ${index} / ${pages}` }} footer={{data}} >
					<Grid width col style={styles.borderBox}>
						<Grid width>
