...
const CabeceraPDF = ({ data = {} } = {}) => {
	const { empresa = {}, liquidaciones = [], ...cabecera } = data;
	...
	const MyPage = ({ children, ...x }) => (
		<Page style={styles.page} size="A4" {...x}>
			<GridView col grow>
				<Header />
	...
	//#region liquidaciones-detalle
	let canvas = document.createElement("canvas");
	pages.push(
		...AsArray(liquidaciones).map(
			({
				establecimiento = {},
				tipoPago = {},
				codigoBarra = "",
						...
						<GridView col style={styles.borderBox}>
							...
							<GridView style={styles.borderTop}>
								...
								<GridView col width style={{ ...styles.paddingBox }}>
									...
									<GridView justify="center">
										<Text>{Formato.Moneda(liquidacion.totalRemuneraciones ?? 0)}</Text>
									</GridView>
								</GridView>
							</GridView>
							<GridView style={styles.borderTop}>
