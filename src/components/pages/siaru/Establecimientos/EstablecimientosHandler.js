...
const EstablecimientosHandler = () => {
  ...
  useEffect(() => {
    ...
    const actions = [
      ...(!route.pathname.includes("Liquidaciones")
        ? [
            createAction({
              action: `Liquidaciones de ${empresa.razonSocial} - ${empresa.id} - ${empresa.cuit}`,
              onExecute: (_) => navigate("Liquidaciones"),
              tarea: "Siaru_EmpresaLiquidaciones",
              keys: "l",
              underlineindex: 0,
            }),
          ]
